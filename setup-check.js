...
console.log(
  `${backendExists ? "✅" : "❌"} Backend directory: ${backendExists ? "Found" : "Missing"}`,
);

// Check if backend package.json exists
const backendPackageExists = fs.existsSync("./backend/package.json");
console.log(
  `${backendPackageExists ? "✅" : "❌"} Backend package.json: ${backendPackageExists ? "Found" : "Missing"}`,
);

// Check if backend .env exists
const backendEnvExists = fs.existsSync("./backend/.env");
console.log(
  `${backendEnvExists ? "✅" : "❌"} Backend .env: ${backendEnvExists ? "Found" : "Missing"}`,
);

// Check if backend node_modules exists
const backendNodeModulesExists = fs.existsSync("./backend/node_modules");
console.log(
  `${backendNodeModulesExists ? "✅" : "❌"} Backend dependencies: ${backendNodeModulesExists ? "Installed" : "Not installed"}`,
);

// Check frontend dependencies
const frontendNodeModulesExists = fs.existsSync("./node_modules");
console.log(
  `${frontendNodeModulesExists ? "✅" : "❌"} Frontend dependencies: ${frontendNodeModulesExists ? "Installed" : "Not installed"}`,
);

// Check environment variable
const envSet = process.env.VITE_API_URL !== undefined;
console.log(
  `${envSet ? "✅" : "❌"} VITE_API_URL environment variable: ${envSet ? "Set" : "Not set"}`,
);
...
if (passed === total) {
  ...
  console.log("   Backend API: https://biniq-backend.onrender.com/api");
  console.log("   Health Check: https://biniq-backend.onrender.com/api/health");
  console.log("\n🔑 Default Admin Login:");
